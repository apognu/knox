...
use git2::{Commit, Config, Cred, IndexAddOption, ObjectType, PushOptions, RemoteCallbacks, Repository, Signature};
...
fn last_commit(repo: &Repository) -> Result<Commit, git2::Error> {
  ...
  object.into_commit().map_err(|_| git2::Error::from_str("could not find latest commit"))
}
  ...
  match Repository::open(&vault.path) {
    Ok(repo) => {
      let (name, email) = Config::open_default()?
        ...
        .map(|c| {
          (
            c.get_string("user.name").unwrap_or_else(|_| "Knox".to_string()),
            c.get_string("user.email").unwrap_or_else(|_| "N/A".to_string()),
          )
        })
    ...
    Err(_) => {
      return Err(VaultError::throw("could not open git repository, data was still written locally"));
    }
  }
...
pub(crate) fn push(vault: &VaultContext) -> Result<(), Box<dyn Error>> {
  ...
  callbacks.credentials(|_, user, credentials| {
    if retry {
      return Err(git2::Error::from_str("ssh agent did not provide valid public key"));
    }
...
mod tests {
  ...
  fn init() {
    ...
    revwalk.set_sorting(Sort::REVERSE).expect("could not set sort");
    ...
    for rev in log {
      let commit = repo.find_commit(rev.unwrap()).expect("could not find root commit");

      assert_eq!(commit.summary().unwrap_or(""), "Initialized knox repository.");
    }
  }
  ...
  fn commit() {
    ...
    context.git_init().expect("could not create local git repository");

    context.write_entry("a", &Entry::default()).expect("could not write entry");
    context.commit("abcdef").expect("could not commit changes");

    let repo = Repository::open(tmp.path()).expect("could not open repository");
    let mut revwalk = repo.revwalk().expect("could not get revwalk");
    revwalk.push_head().expect("could not find HEAD");
    revwalk.set_sorting(Sort::REVERSE).expect("could not get GPG key");
  ...
  fn set_remote() {
    ...
    let repo = Repository::open(tmp.path()).expect("could not open repository");
    repo.remote("testremote", "https://git.example.com").expect("could not set remote URL");

    let remote = repo.find_remote("testremote").expect("could not find created remote");
