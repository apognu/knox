...
fn check_entry(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
  ...
  for (name, attribute) in pwnage {
    match attribute {
      PwnedResult::Error(err) => println!(" {} {} {}:{} ({})", "::".magenta().bold(), "ERROR".magenta(), path.bold(), name.dimmed(), err),

      PwnedResult::Clear => println!(" {} {} {}:{}", "::".green().bold(), "CLEAR".green(), path.bold(), name.dimmed()),

      PwnedResult::Pwned => println!(" {} {} {}:{}", "::".red().bold(), "PWNED".red(), path.bold(), name.dimmed()),
    }
  }
        ...
        if let AttributeValue::String(value) = attribute.value() {
          match check(&value) {
            PwnedResult::Pwned => {
              ...
              progress.println(format!(" {} {} {}:{}", "::".red().bold(), "PWNED".red(), path.bold(), name.dimmed()));
            }

            PwnedResult::Error(err) => {
              progress.println(format!(" {} {} {}:{} ({})", "::".red().bold(), "ERROR".red(), path.bold(), name.dimmed(), err));
            }
...
pub(crate) fn check_attributes(attributes: &HashMap<String, Attribute>) -> Vec<(String, PwnedResult)> {
  attributes
    .iter()
...
fn check(value: &str) -> PwnedResult {
  ...
  match client {
    Err(err) => return PwnedResult::Error(err.to_string()),
    Ok(mut response) => {
...
mod tests {
  ...
  fn check() {
    ...
    let secure = rand::thread_rng().sample_iter(&Alphanumeric).take(64).collect::<String>();
